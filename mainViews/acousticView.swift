...
import AVFoundation
import UIKit
struct acousticView: View {
    var gradient: [Color] = [Color("Acoustic"), Color("Acoustic2")]
...
enum ActiveAlertAcoustic {
    case first, second
}

struct acoustic1: View {
    ...
    @State private var activeAlert: ActiveAlertAcoustic = .first
    @State private var activeAlertTwo: ActiveAlertAcoustic = .second
    ...
    @ObservedObject var acousticsettings = acousticSettings()
    ...
    var body: some View {
        VStack (alignment: .leading, spacing: 20) {
            ...
            Text("Song Title:")
                ...
                .foregroundColor(Color.white)
        TextView(text: $acousticsettings.songTitleAcousticOne, textStyle: $textStyle)
                //            .padding()
            ...
            Text("Song Lyrics:")
                .font(Font.custom("Lobster 1.4", size: 22))
                .foregroundColor(Color.white)
            TextView(text: $acousticsettings.songLyricsAcousticOne, textStyle: $textStyle)
                //      .padding(10)
                .background(Color(UIColor.tertiarySystemFill))
                .cornerRadius(9)
            ...
            HStack {
                ...
                Button(action: {
                    ...
                    let fileName = path.appendingPathComponent("Acoustic Recording.caf")
                    ...
                    if (self.recordButton == Image(systemName: "mic.circle")) {
                        self.recordingsettings.setUpAudio(sound: "Acoustic 7.caf")
                        self.recordingsettings.timer = Timer.publish(every: 1, on: .main, in: .common).autoconnect()
                        self.recordingsettings.playSoundThree(sound: "Acoustic 7", type: "caf")
                        self.recordingsettings.songDuration = 160.00
                        let currentTime = self.recordingsettings.audioPlayerThree?.currentTime
                        self.recordingsettings.playValue = currentTime!
                        self.recordingsettings.timer = Timer.publish(every: 1, on: .main, in: .common).autoconnect()

                        try! self.recordingsettings.startRecording(sound: "Acoustic Recording.caf")
            ...
            //            showingAlertHeadphones = true
                ...
                .alert(isPresented: $showAlert) {
                    ...
                    case .second:
                        return Alert(title: Text("Are you sure you want to overwrite your acoustic expression?"), primaryButton: .destructive(Text("Let's go!")){
                            if (self.recordButton == Image(systemName: "mic.circle")) {
             ...
             Button(action: {
                ...
                if self.playbackButton == Image(systemName: "waveform.circle") {
                    ...
                    self.recordingsettings.playSoundTwo(sound: "Acoustic Recording.caf")
...
struct StudyBeatsProAcoustic: View {
...
struct acousticView_Previews: PreviewProvider {
