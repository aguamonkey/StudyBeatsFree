...
import AVFoundation
import UIKit
struct reggaeView: View {
    var gradient: [Color] = [Color("Reggae"), Color("Reggae2")]
...
struct reggae1: View {
    ...
    @State private var activeAlert: ActiveAlertReggae = .first
    @State private var activeAlertTwo: ActiveAlertReggae = .second
    ...
    @ObservedObject var reggaesettings = reggaeSettings()
    ...
    var body: some View {
        VStack (alignment: .leading, spacing: 20) {
            ...
            Text("Song Title:")
                ...
                .foregroundColor(Color.white)
        TextView(text: $reggaesettings.songTitleReggaeOne, textStyle: $textStyle)
                //            .padding()
            ...
            Text("Song Lyrics:")
                .font(Font.custom("Lobster 1.4", size: 22))
                .foregroundColor(Color.white)
            TextView(text: $reggaesettings.songLyricsReggaeOne, textStyle: $textStyle)
                //      .padding(10)
                .background(Color(UIColor.tertiarySystemFill))
                .cornerRadius(9)
            ...
            //MARK:- PLAYBACK CONTROLS / BUTTONS
            ...
            HStack {
                ...
                Button(action: {
                    ...
                    let fileName = path.appendingPathComponent("Reggae Recording.caf")
                    ...
                    if (self.recordButton == Image(systemName: "mic.circle")) {
                        self.recordingsettings.setUpAudio(sound: "Reggae 9.caf")
                        self.recordingsettings.timer = Timer.publish(every: 1, on: .main, in: .common).autoconnect()
                        self.recordingsettings.playSoundThree(sound: "Reggae 9", type: "caf")
                        self.recordingsettings.songDuration = 186.00
                        let currentTime = self.recordingsettings.audioPlayerThree?.currentTime
                        self.recordingsettings.playValue = currentTime!
                        self.recordingsettings.timer = Timer.publish(every: 1, on: .main, in: .common).autoconnect()

                        try! self.recordingsettings.startRecording(sound: "Reggae Recording.caf")
            ...
            //            showingAlertHeadphones = true
                ...
                .alert(isPresented: $showAlert) {
                    ...
                    case .second:
                        return Alert(title: Text("Are you sure you want to overwrite your reggae rendition?"), primaryButton: .destructive(Text("Let's go!")){
                            if (self.recordButton == Image(systemName: "mic.circle")) {
             ...
             Button(action: {
                ...
                if self.playbackButton == Image(systemName: "waveform.circle") {
                    ...
                    self.recordingsettings.playSoundTwo(sound: "Reggae Recording.caf")
...
struct StudyBeatsProReggae: View {
...
struct reggaeView_Previews: PreviewProvider {
