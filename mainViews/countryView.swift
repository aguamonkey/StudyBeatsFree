...
struct countryView: View {
    var gradient: [Color] = [Color("Country"), Color("Country2")]
...
enum ActiveAlertCountry {
    case first, second
}

struct country1: View {
    ...
    @State private var activeAlert: ActiveAlertCountry = .first
    @State private var activeAlertTwo: ActiveAlertCountry = .second
    ...
    @ObservedObject var countrysettings = countrySettings()
    ...
    var body: some View {
        VStack (alignment: .leading, spacing: 20) {
            ...
            Text("Song Title:")
                ...
                .foregroundColor(Color.white)
        TextView(text: $countrysettings.songTitleCountryOne, textStyle: $textStyle)
                //            .padding()
            ...
            Text("Song Lyrics:")
                .font(Font.custom("Lobster 1.4", size: 22))
                .foregroundColor(Color.white)
            TextView(text: $countrysettings.songLyricsCountryOne, textStyle: $textStyle)
                //      .padding(10)
                .background(Color(UIColor.tertiarySystemFill))
                .cornerRadius(9)
            ...
            HStack {
                ...
                Button(action: {
                    ...
                    let fileName = path.appendingPathComponent("Country Recording.caf")
                    ...
                    if (self.recordButton == Image(systemName: "mic.circle")) {
                        self.recordingsettings.setUpAudio(sound: "Country 6.caf")
                        self.recordingsettings.timer = Timer.publish(every: 1, on: .main, in: .common).autoconnect()
                        self.recordingsettings.playSoundThree(sound: "Country 6", type: "caf")
                        self.recordingsettings.songDuration = 150.00
                        let currentTime = self.recordingsettings.audioPlayerThree?.currentTime
                        self.recordingsettings.playValue = currentTime!
                        self.recordingsettings.timer = Timer.publish(every: 1, on: .main, in: .common).autoconnect()

                        try! self.recordingsettings.startRecording(sound: "Country Recording.caf")
            ...
            //            showingAlertHeadphones = true
                ...
                .alert(isPresented: $showAlert) {
                    ...
                    case .second:
                        return Alert(title: Text("Are you sure you want to overwrite your country anthem?"), primaryButton: .destructive(Text("Let's go!")){
                            if (self.recordButton == Image(systemName: "mic.circle")) {
             ...
             Button(action: {
                ...
                if self.playbackButton == Image(systemName: "waveform.circle") {
                    ...
                    self.recordingsettings.playSoundTwo(sound: "Country Recording.caf")
