...
import UIKit
struct rapView: View {
    var gradient: [Color] = [Color("Rap"), Color("Rap2")]
...
struct rap1: View {
    ...
    @State private var activeAlert: ActiveAlertRap = .first
    @State private var activeAlertTwo: ActiveAlertRap = .second
    ...
    @ObservedObject var rapsettings = rapSettings()
    ...
    var body: some View {
        VStack (alignment: .leading, spacing: 20) {
            ...
            Text("Song Title:")
                ...
                .foregroundColor(Color.white)
        TextView(text: $rapsettings.songTitleRapOne, textStyle: $textStyle)
                //            .padding()
            ...
            Text("Song Lyrics:")
                .font(Font.custom("Lobster 1.4", size: 22))
                .foregroundColor(Color.white)
            TextView(text: $rapsettings.songLyricsRapOne, textStyle: $textStyle)
                //      .padding(10)
                .background(Color(UIColor.tertiarySystemFill))
                .cornerRadius(9)
            ...
            HStack {
                ...
                Button(action: {
                    ...
                    let fileName = path.appendingPathComponent("Rap Recording.caf")
                    ...
                    if (self.recordButton == Image(systemName: "mic.circle")) {
                        self.recordingsettings.setUpAudio(sound: "Rap 4.caf")
                        self.recordingsettings.timer = Timer.publish(every: 1, on: .main, in: .common).autoconnect()
                        self.recordingsettings.playSoundThree(sound: "Rap 4", type: "caf")
                        self.recordingsettings.songDuration = 166.00
                        let currentTime = self.recordingsettings.audioPlayerThree?.currentTime
                        self.recordingsettings.playValue = currentTime!
                        self.recordingsettings.timer = Timer.publish(every: 1, on: .main, in: .common).autoconnect()

                        try! self.recordingsettings.startRecording(sound: "Rap Recording.caf")
            ...
            //            showingAlertHeadphones = true
                ...
                .alert(isPresented: $showAlert) {
                    ...
                    case .second:
                        return Alert(title: Text("Are you sure you want to recycle your rap flow?"), primaryButton: .destructive(Text("Let's go!")){
                            if (self.recordButton == Image(systemName: "mic.circle")) {
             ...
             Button(action: {
                ...
                if self.playbackButton == Image(systemName: "waveform.circle") {
                    ...
                    self.recordingsettings.playSoundTwo(sound: "Rap Recording.caf")
...
struct rapView_Previews: PreviewProvider {
